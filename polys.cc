...
#ifdef DEBUG
		cout<<endl<<"p["<<i<<"]: "<<ptV[i]->x<<","<<ptV[i]->y<<", K:"<<K<<endl;
#endif		
...
void polys::matrixPaint ( const std::vector< std::vector<float> > &deltaVV,
	...
	for (int ii=0; ii<deltaVV.size(); ++ii){
		...
		for ( int jj=0; jj<deltaV.size() ; ++jj ){
			cout<<deltaV[jj]<<",";
			if ( deltaV[jj]>highest ){
				highest=deltaV[jj];
				cout<<endl<<"highest:"<<highest<<endl;
			}
			if ( deltaV[jj]<lowest )	lowest=deltaV[jj];
		}
	}
	...
	cout<<endl<<"highest="<<highest<<", lowest="<<lowest
		<<", deltaV.size():"<<deltaVV.size()<<endl;
		
	float offset=log(lowest);
	float dist=log(highest)-offset;
	float coef= 255/dist;
	
	cout<<"offset:"<<offset<<", dist:"<<dist<<", coef:"<<coef<<endl;
	for (int ii=0; ii<deltaVV.size(); ++ii ){
		vector <float>	deltaV(deltaVV[ii]);
		for( int jj=0; jj< deltaV.size(); ++jj){
			cvSetReal2D ( tgray, jj, ii, (log(deltaV[jj])-offset)*coef );
		}
	}
